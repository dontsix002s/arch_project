...
namespace i2c::backend::stm32h7::ll {
    ...
    static Error write_read(uint16_t addr,
	    ...
	    static void on_event_isr();
	    static void on_error_isr();
    };
	
	
	
	// (Да, это “header-only” определения — но пока это пустышки, ок. 
	// Когда появится реальная IRQ логика, можно будет перенести в .tpp или .cpp.)
	
	template <const PeriphDescriptor& Periph, typename Policy, typename Wait>
		void LlBackend<Periph, Policy, Wait>::on_event_isr()
		{
			// TODO: implement when IrqPolicy/DmaIrqPolicy is supported
		}

	template <const PeriphDescriptor& Periph, typename Policy, typename Wait>
		void LlBackend<Periph, Policy, Wait>::on_error_isr()
		{
			// TODO: implement when IrqPolicy/DmaIrqPolicy is supported
		}
